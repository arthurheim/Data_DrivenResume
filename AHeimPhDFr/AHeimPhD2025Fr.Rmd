...
document: 
qualifications: PhD
position: "PhD -- Analyse et politique économique (APE)"
pronouns: Il/Iel
...
linkcolor: "ff0080"
urlcolor: "0000ff" 
aboutme: "Je suis économiste appliqué, spécialisé dans la conception et l’évaluation des politiques publiques visant à réduire les inégalités et la pauvreté en France. J'ai reçu le prix de thèse 2025 de la Cour des comptes. Mon travail repose principalement sur des expériences randomisées et la microéconométrie pour l’évaluation des politiques."
header-includes:
  \usepackage{fontspec}
  \usepackage{float}
  \usepackage{starfont}
...
readExcelSheets(here::here("Data", "ArthurHeimCareerDataFr.xlsx"))
...
\section{Éxperiences $\quad$ \faSuitcase}
...
\section{Études et diplômes $\quad$ \faInstitution }
...
\section{Distinctions $\quad$ \faTrophy }
...
\section{Gestion de projets $\quad$ \faLaptop}

\subsection{Expérimentations de terrain}

<!-- ### Completed {-} -->

```{r}
 ResearchProjects %>%
  filter(type %in% c("Expérimentation aléatoire")) %>%
...
```{r include=TRUE}
 ResearchProjects %>%
  filter(type %in% c("Expérimentation aléatoire","Market design","Conception de politique publique" )==FALSE) %>% 
  detailed_entries(with = Project,
                   when = glue::glue("{start_date}--{end_date}"),
                   what = institution,
                   where = where,
...
\section{Éxpertise, conseil et formations $\quad$ \faBarChartO}
...
\section{Enseignements $\quad$ \faGroup}
...
\section{Referee reports $\quad$ \faCommenting}

Journal of Economic Policy and Management (JEPM), Journal of Labor policy (JOLP), La revue économique (RECO), Revue des politiques sociales et familiales (RPSF), Revue Française de Pédagogie, Éducation et Formation
...
PapersECEC <- PaperDetails %>%
  filter(bibtex_ID %in% c("CarbucciaEtAl2025","CombeHeim2024")) %>% as.tibble()  %>% arrange(desc(status))%>%
...
##	Politiques d'activation des familles monoparentales
...
\clearpage
## Travail collectif sur la replicabilité des recherches 

```{r}
PaperDetails %>%
  filter(bibtex_ID %in% "multi1002025") %>%
  detailed_entries(with = title,
when = status,
what = coauthors %>% replace_na(.,""),
where = glue::glue("{end_date}"),
...
```
<!-- \clearpage -->
### Documents de travail et notes d'analyses  {-}
...
<!-- \clearpage -->
\section{Sélection de conférences $\quad$ \faInstitution}


```{r}
conferences %>% filter(include=="yes")%>%
...
\section{Langues $\quad$ \faLanguage}
...
<!-- \clearpage -->
\section{Programmation $\quad$ \faConnectdevelop}
