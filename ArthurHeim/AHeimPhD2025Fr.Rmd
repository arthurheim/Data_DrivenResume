...
linkcolor: "ff0080"
urlcolor: "0000ff" 
aboutme: "Je suis économiste appliqué, spécialisé dans la conception et l’évaluation des politiques publiques visant à réduire les inégalités et la pauvreté en France. Mon travail repose principalement sur des expériences randomisées et la microéconométrie pour l’évaluation des politiques. Je pilote également un programme utilisant des mécanismes de marché pour l’attribution des places en crèche dans plusieurs municipalités françaises."
header-includes:
  \usepackage{fontspec}
  \usepackage{float}
  \usepackage{starfont}
...
```{r include=TRUE}
 ResearchProjects %>%
  filter(type %in% c("Market design","Conception de politique publique")) %>% 
  detailed_entries(with = Project,
                   when = glue::glue("{start_date}--{end_date}"),
                   what = institution,
                   where = where,
...
# Éxpertise, conseil et formations
